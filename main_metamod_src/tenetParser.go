...
func writeTenetScriptedEffect(inDir string, outDir string, outName string) []inno {
	innoLib := readTenetFiles(inDir)
	...
	writeHeader(outFile)
	_, err = outFile.WriteString("demd_add_doctrine_bonuses = {\n\n")
...
func readTenetFiles(inDir string) []inno {
	...
	for i := 0; i < len(fileInfo); i++ {
		innos := readTenetFile(filepath.Join(inDir,fileInfo[i].Name()))
...
func readTenetFile(inFile string) map[string]inno {
	...
	for scanner.Scan() {
		...
		if len(fields) > 0 && bracketCounter == 1 && (strings.Split(fields[0], "_")[0] == "doctrine" || strings.Split(fields[0], "_")[0] == "tenet") {
		...
		if len(fields) > 2 && strings.Contains(fields[0],"demd_meta") {
			...
			if fragments[length-2] == "mult" {
				value = value / 100.0
				resource += "_faith_production_mult"
